...
import { ArcStatementForm } from '@/components/profile/arc-statement-form';
 ...
 * preference, the arc statement (G-005 - the athlete's own one-liner on why
 * the goal race matters, shown on Patrol), the injury/illness ledger, and
 * (Stage 5) every source Connection the coach can read from. Where generic
 * gives way to personal. Daily wellness check-in moved to the Journal page
 * (Stage 3 - interruption log + wellness merged into one surface).
 */
export default async function ProfilePage() {
  logPageView('/profile');
  const [profile, nsCalibration, strength, adapterStatuses, promptDefaults, timezone, arcStatement] = await Promise.all([
    getAthleteProfile(),
    getNsHrCalibration(),
    getStrengthPreferences(),
    getAdapterStatuses(),
  ...
  return (
    <div className="px-4 sm:px-8 lg:px-12 py-8 sm:py-10 max-w-5xl mx-auto space-y-8">
      <header className="border-b border-ink-line pb-6 space-y-1">
        ...
        <h1 className="font-display tracking-wide-display text-5xl uppercase">Profile</h1>
      ...
      <ArcStatementForm initial={arcStatement} maxLength={ARC_STATEMENT_MAX_LENGTH} />
